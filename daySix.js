/* Part 1:  Because your neighbors keep defeating you in the holiday house decorating contest year after year, you've decided to deploy one million lights in a 1000x1000 grid.
...
Part 2: You just finish implementing your winning light pattern when you realize you mistranslated Santa's message from Ancient Nordic Elvish.
...
function SingleInput(startingX, startingY, endingX, endingY, state){
...
var buildArray = function(x, y, state) {
	for (var i = 0; i <= x; i++) {
    	lights[i] = [];
    	for (var j = 0; j <= y; j++) {
        	lights[i][j] = Coords(i, j, state);
    	}
	}
};

var numberFilter = function(value) {
...
var parseInput = function(input) {
	...
	for (var i = 0; i < splitInput.length; i++) {
		...
		for (var k = 0; k < numbersInput.length; k++) {
			var currentNum = Number(numbersInput[k]);
			switch (k) {
				case 0:
		...
		finalInput.push(instruction);
	}
};

var changeState = function(instruction) {
	if (instruction.state === "toggle") {
		...
		for (var i = instruction.startingX; i <= instruction.endingX; i++) {
			for (var j = instruction.startingY; j <= instruction.endingY; j++) {
				lights[i][j].state = instruction.state;
			}
		}
	}
};
...
var partOne = function(lengthOfX, lengthOfY, initialState, input) {
	parseInput(input);
