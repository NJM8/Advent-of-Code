/*Part 1:  Santa is delivering presents to an infinite two-dimensional grid of houses.
...
Note: Code is currently set to answer part 2.
...
// global data object for tracking houses visited and each Santa's current location
var data = {
	housesVisited: [[0, 0]], 
	humanSantaLocation: [0, 0],
	roboSantaLocation: [0, 0],

	addHouse : function(whichSanta) {
		if (whichSanta == 'humanSanta') {
			this.housesVisited.push([this.humanSantaLocation[0], this.humanSantaLocation[1]]);
		} else if (whichSanta == 'roboSanta') {
			this.housesVisited.push([this.roboSantaLocation[0], this.roboSantaLocation[1]]);
		}
	},

// function to find the next X,Y location based on the current location and a direction
	nextLocation : function(direction, whichSanta) {
		if (whichSanta == 'humanSanta') {
			switch (direction) {
			case '>':
				[this.humanSantaLocation[0] += 1, this.humanSantaLocation[1]];
				break;
			case '<':
				[this.humanSantaLocation[0] -= 1, this.humanSantaLocation[1]];
				break;
			case '^':
				[this.humanSantaLocation[0], this.humanSantaLocation[1] += 1];
				break;
			case 'v':
				[this.humanSantaLocation[0], this.humanSantaLocation[1] -= 1];
				break;
			...
			switch (direction) {
			case '>':
				[this.roboSantaLocation[0] += 1, this.roboSantaLocation[1]];
				break;
			case '<':
				[this.roboSantaLocation[0] -= 1, this.roboSantaLocation[1]];
				break;
			case '^':
				[this.roboSantaLocation[0], this.roboSantaLocation[1] += 1];
				break;
			case 'v':
				[this.roboSantaLocation[0], this.roboSantaLocation[1] -= 1];
				break;
...
// helper function to check if the current location has already been visited, returns true or false
	checkHousesVisited : function(whichSanta) {
		if (whichSanta == 'humanSanta') {
			for (var i = 0; i < this.housesVisited.length; i++) {
				if (this.humanSantaLocation[0] === this.housesVisited[i][0] && 
					this.humanSantaLocation[1] === this.housesVisited[i][1]) {
					return true;
				} else if (this.humanSantaLocation[0] !== this.housesVisited[i][0] && 
					this.humanSantaLocation[1] !== this.housesVisited[i][1]) {
					continue;
				}
			}
			...
			for (var i = 0; i < this.housesVisited.length; i++) {
				if (this.roboSantaLocation[0] === this.housesVisited[i][0] && 
					this.roboSantaLocation[1] === this.housesVisited[i][1]) {
					return true;
				} else if (this.roboSantaLocation[0] !== this.housesVisited[i][0] && 
					this.roboSantaLocation[1] !== this.housesVisited[i][1]) {
					continue;
				}
			}
...
// main runner function, parse the directions into comma seperated strings, walk through the list, if it is divisible 
// by 2 increment humanSanta, if not increment roboSanta, return the length of houses visited
var main = function(input) {
	...
	for (var i = 0; i < directions.length; i++) {
		if (i % 2 === 0) {
			data.nextLocation(directions[i], 'humanSanta');
			if (data.checkHousesVisited('humanSanta') === false) {
				data.addHouse('humanSanta');
			}
		} else {
			data.nextLocation(directions[i], 'roboSanta');
			if (data.checkHousesVisited('roboSanta') === false) {
				data.addHouse('roboSanta');
			}
		}
	}

	return data.housesVisited.length;
};
